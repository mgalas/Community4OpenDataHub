...
function qa_get_login_links($relative_url_prefix, $redirect_back_to_url)
{
	...
	$address = 'http://ucl-project.dd:8083';
	return array(
		'login' => $address.'/user/login',
		'register' => $address.'/user/register',
		'logout' => $address.'/user/logout'
	);
...
function qa_get_logged_in_user()
{
	...
	*/
		...
		if (isset($_COOKIE['Drupal_visitor_is_logged_in'])) {

			$userid = $_COOKIE['Drupal_visitor_logged_in_userid'];

			$result = qa_db_read_one_assoc(qa_db_query_sub(
				'SELECT name, mail FROM users_field_data WHERE uid=$',
				$userid
			));
			...
			if (is_array($result)) {
				return array(
					...
					'email' => $result['mail'],
					'level' => (strtolower($result['name']) == 'admin') ? QA_USER_LEVEL_ADMIN : QA_USER_LEVEL_BASIC
				);
			}
		}
